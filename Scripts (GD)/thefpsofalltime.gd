class_name thefpsofalltime extends CharacterBody3D
@onready var lol: bool = true
@export var rotation_speed: Vector3 = Vector3(0.6, 0, 0) # Degrees per second
@export var thing: Node3D
@export var target: Node # Assign the path to the leading node in the Inspector
...
func _process(delta: float) -> void:
	# Camera Lock
	if Input.is_action_just_pressed(InputDictionary["Escape"]) && _isMouseCaptured:
		Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
		_isMouseCaptured = false
	elif Input.is_action_just_pressed(InputDictionary["Escape"]) && !_isMouseCaptured && !VAR.is_interacting:
		Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
...
# Attach to a node that manages cameras
func switch_to_camera(target_camera: Camera3D):
	var tween = create_tween()
	# Smoothly move to new camera position/zoom
	tween.tween_property(camera, "global_position", target_camera.global_position, 0.5).set_trans(Tween.TRANS_SINE)
	tween.parallel().tween_property(camera, "zoom", target_camera.zoom, 0.5)
	await tween.finished
	target_camera.make_current() # Make final camera active
